...
export function NetworkGate({ blockChildren = false, children }: Props) {
  const { isConnected, isWrongNetwork, needsNetworkSwitch, targetChainId, walletChainId, chainId } =
    useDeployment();
  ...
  const currentName = chainId != null ? CHAIN_BY_ID[chainId]?.name : `chain ${walletChainId}`;
