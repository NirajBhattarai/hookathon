...
function TokenAvatar({ color, symbol }: { color?: string; symbol: string }) {
  return (
    <span className="token-avatar token-avatar-sm" style={color ? { background: color } : undefined}>
      {symbol.slice(0, 1)}
    </span>
  )
}
...
export function SwapForm() {
  ...
  const [c0, c1] = useMemo((): readonly [Address?, Address?] => {
    ...
    return inAddr.toLowerCase() < outAddr.toLowerCase()
      ? [inAddr, outAddr]
      : [outAddr, inAddr]
  ...
  const discoverQ = useReadContracts({
    ...
    contracts: tickCandidates.map((ts) => ({
      ...
      abi: [{ type: 'function', name: 'isConfigured', stateMutability: 'view', inputs: [{ name: '', type: 'bytes32' }], outputs: [{ name: '', type: 'bool' }] }] as const,
  ...
  const discoveredTickSpacing = useMemo(
    () => tickCandidates[discoverQ.data?.findIndex((r) => r.status === 'success' && r.result === true) ?? -1],
